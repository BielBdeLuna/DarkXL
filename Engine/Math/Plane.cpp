...
HalfSpace_e Plane::ClassifyPoint(const Vector3& vPt)
{
	float dist;
...
HalfSpace_e PlaneD::ClassifyPoint(const dVector3& vPt)
{
	double dist;
	dist = a*vPt.x + b*vPt.y + c*vPt.z + d;
	if ( dist < 0.0 ) return NEGATIVE;
	if ( dist > 0.0 ) return POSITIVE;
	return ON_PLANE;
}

void PlaneD::Build(dVector3 *vPoints)
{
	dVector3 vA, vB, vN;
	vA.Set( vPoints[1].x-vPoints[0].x, vPoints[1].y-vPoints[0].y, vPoints[1].z-vPoints[0].z );
	vB.Set( vPoints[2].x-vPoints[0].x, vPoints[2].y-vPoints[0].y, vPoints[2].z-vPoints[0].z );
...
bool PlaneD::Build(const dVector3& v0, const dVector3& v1, const dVector3& v2)
{
	dVector3 vA, vB, vN;
	vA.Set(v1.x-v0.x, v1.y-v0.y, v1.z-v0.z);
	vB.Set(v2.x-v0.x, v2.y-v0.y, v2.z-v0.z);
